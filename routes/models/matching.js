...
from (select truc.user_id, count(client.tag_id) as count, GROUP_CONCAT(ini_tags.label SEPARATOR ' ') as concat
	from tag_user as client
...
select user_id, count(tag_id) as count, GROUP_CONCAT(label SEPARATOR ' ') as concat
	from tag_user
inner join tags
...
where users.gender in ("femme", "homme") and users.id < 800 and users.id != 11) as diff2) as mint
	on 1 = 1) as tab_dist
left join (select concat, tags.user_id, tags.count as countsugg, tot.count as counttot, client.count as countclient, tags.count / ((tot.count + client.count)/2) as restag
from (select user_id, count(tag_id) as count, GROUP_CONCAT(label SEPARATOR ' ') as concat
	from tag_user
inner join tags
	on tag_user.tag_id = tags.id
where find_in_set(label, "stable,mobile")
group by tag_user.user_id) as tab_tag
on tab_dist.suggid = tab_tag.user_id;
...
inner join tags
	on tag_user.tag_id = tags.id
where find_in_set(tags.label, "'stable', 'mobile'")
group by tag_user.user_id;
...
from users right join (select id as client, lat as latc, lon as lonc from users where id = 11) as test2 on 1 = 1 
where users.gender in ("femme", "homme") and users.id < 800 and users.id != 11) as diff2) as mint on 1 = 1) as tab_dist
left join (select user_id, count(tag_id) as restag, GROUP_CONCAT(label SEPARATOR ' ') as concat
	from tag_user
inner join tags
	on tag_user.tag_id = tags.id
where find_in_set(label, "")
group by tag_user.user_id) as tab_tag on tab_dist.suggid = tab_tag.user_id;
